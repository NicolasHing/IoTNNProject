...
void cluster_delegate(void *arg)
{
    ...
    int t1_start = pi_time_get_us();
    printf("\nstart %llu\n",t1_start);
    pi_perf_start();
    
    /* Task dispatch to cluster cores. */
    // pi_cl_team_fork(numberOfCores, task to send to core, arg);
    pi_cl_team_fork(1, trainingNN, arg);
    pi_perf_stop();
    
    int t1_end = pi_time_get_us();
    printf("\nend %llu\n",t1_end);
    ...
    int t2_start = pi_time_get_us();
    printf("\nstart %llu\n",t2_start);
    pi_perf_start();
    
    /* Task dispatch to cluster cores. */
    // pi_cl_team_fork(numberOfCores, task to send to core, arg);
    pi_cl_team_fork(1, testNN, arg);
    
    pi_perf_stop();
    
    int t2_end = pi_time_get_us();
    printf("\nend %llu\n",t2_end);
